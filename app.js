...
app.post('/developers', (req, res) => { // CREATE
  ...
  } else if (!newDeveloper.skills) {
    res.status(400).send('Developer skills is required')
  } else if (!Array.isArray(newDeveloper.skills) || newDeveloper.skills.length < 0) {
...
app.put('/developers/:id', (req, res) => { // CREATE
  ...
  if (replacedDeveloper && replacedDeveloperId) {
    ...
    } else if (!Array.isArray(req.body.skills) || req.body.skills.length < 0) {
